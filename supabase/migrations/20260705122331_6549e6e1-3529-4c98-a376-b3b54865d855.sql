...
CREATE POLICY "Members can view workspace invitations"
  ...
  USING (public.user_has_workspace_access(workspace_id) OR lower(email) = lower(auth.jwt() ->> 'email'));
...
DROP POLICY "Invitee can join via valid invitation" ON public.workspace_members;
