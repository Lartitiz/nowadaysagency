-- Fix "faux succès" in redeem-promo: the edge function was awaiting several
-- decisive writes (usage counter, profile plan, subscription, coaching
-- program + sessions + deliverables) without checking { error }, so a
-- mid-sequence failure still returned { success: true } while leaving the
-- client with a half-granted plan or an empty "Binôme" space.
--
-- Both functions below run as a single Postgres statement, so Postgres
-- rolls back every write inside them automatically if any step fails —
-- no more hand-rolled compensating deletes in the edge function, and no
-- more "coaching program exists but has 0 sessions" partial state.

-- 1. Atomically record the redemption, bump the usage counter (re-checking
--    max_uses under the row lock, so a limited code can never be over-redeemed
--    even if two requests race past the edge function's earlier check), and
--    grant the plan on both profiles and subscriptions.
...
REVOKE EXECUTE ON FUNCTION public.redeem_promo_and_grant_plan(uuid, uuid, text, text, timestamptz) FROM anon, authenticated;

-- 2. Atomically create the coaching program together with its 9 sessions and
--    10 deliverables. Returns the new program id, or NULL if an active
--    program already exists for this client (no-op, same as before).
...
REVOKE EXECUTE ON FUNCTION public.create_coaching_program_full(uuid, uuid, date, date, text) FROM anon, authenticated;
