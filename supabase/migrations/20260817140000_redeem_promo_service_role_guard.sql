-- SECURITY FIX (found while verifying 20260817120000_redeem_promo_atomic_grant.sql live):
-- REVOKE EXECUTE ... FROM anon, authenticated does NOT actually block calls through
-- PostgREST on this project — confirmed both on the two functions just added AND on
-- the pre-existing increment_promo_uses (revoked back on 2026-04-29), by calling all
-- three directly with only the public anon apikey and no user session. Root cause is
-- some default-privilege grant at the platform/schema level that the REVOKE doesn't
-- override; that's a separate, wider audit (increment_bonus_credits and the other
-- functions revoked in that same migration are very likely equally exposed).
--
-- For these two functions specifically — which grant a paid plan / create a coaching
-- program — that gap means anyone holding the public anon key could call them directly
-- with arbitrary user_id/plan, bypassing every check the edge function performs
-- (promo validity, expiry, max_uses, ownership). This adds a defense-in-depth guard
-- inside the function body itself, checked against the JWT role claim rather than the
-- grant system, so it holds regardless of why the REVOKE isn't effective.
