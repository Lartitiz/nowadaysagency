...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "title") {
      ...
      systemPrompt = `${LINKEDIN_PRINCIPLES}\n\n${context}\n\n${qualityBlocks}\n\nÉLÉMENTS FOURNIS :\n- Sa passion : "${passion || ""}"\n- Son parcours : "${parcours || ""}"\n- Ce qu'elle propose : "${offre || ""}"\n- Son appel à l'action : "${cta || ""}"\n\nRÈGLES CRITIQUES :\n- Les 265-275 premiers caractères sont visibles avant "voir plus". Le hook DOIT être dans ces 265 premiers caractères.\n- Max 2 600 caractères au total.\n- Intégrer naturellement des mots-clés SEO (LinkedIn indexe cette section pour les recherches).\n- Aérer avec des sauts de ligne.\n- Le résumé développe ce que le titre promet : cohérence obligatoire.\n- PAS de "Bonjour, je suis [prénom] et je suis passionnée par..." : c'est le hook le plus faible possible.\n\nGénère 2 versions du résumé LinkedIn :\n\nVERSION STORYTELLING :\n- Hook percutant, parcours narratif, proposition de valeur naturelle, CTA clair\n- 200-300 mots\n${branding.storytelling?.step_7_polished ? "- Utilise le storytelling comme base narrative" : ""}\n\nVERSION PRO :\n- Plus structurée, factuelle\n- 150-200 mots\n\nRéponds UNIQUEMENT en JSON sans backticks :\n{"storytelling": "...", "pro": "..."}`;
