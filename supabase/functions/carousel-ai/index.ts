...
═══ PRINCIPE Nº1 — AVANT TOUTE AUTRE RÈGLE ═══
Écris le carrousel comme UNE SEULE histoire parlée, fluide, du début à la fin — comme si tu la racontais à voix haute d'un trait à une amie. PUIS découpe ce texte en overlays, un morceau par slide. On doit TOUJOURS savoir qui parle et de qui on parle (plante les personnages avant les pronoms). Le test ultime : lus d'affilée, les overlays sonnent comme un récit naturel et continu — pas comme des slides assemblées à partir d'une liste de règles, pas comme des phrases étiquetées par un surtitre. Toutes les règles ci-dessous SERVENT ce principe : si l'une te pousse vers un rendu mécanique (ex. un connecteur plaqué sur chaque slide), la fluidité prime.
...
═══ ANCRAGE DES PERSONNAGES — QUI PARLE, DE QUI ═══
Le lecteur doit TOUJOURS savoir qui parle (la narratrice) et de qui on parle. Un récit qui coule mais dont on ignore les personnages = raté (ex vécu : slide 1 "Tout le monde te parle de technique…" puis slide 2 "La première conversation qu'ON a eue…" → on ne sait pas avec QUI).
- Si l'histoire met en scène quelqu'un (une cliente, un binôme, un cas), PLANTE-le explicitement AVANT le premier pronom qui le désigne : nomme-le ou pose la scène dès la slide 1 ou 2 ("Quand une cliente débarque bloquée…", "L'autre jour, avec une cliente…", "Une créatrice m'écrit : 'je bloque'…").
- VOIX COHÉRENTE : ne glisse pas sans transition d'un "te/tu/vous" (qui s'adresse au lecteur) à un "on/elle/il" (qui raconte une scène). Choisis ton mode et tiens-le. Si tu pars d'une accroche qui interpelle le lecteur, la slide suivante DOIT installer clairement la scène/le personnage avant de filer le récit.
- Le "je" de la narratrice peut porter le récit, mais alors les autres personnages ("ma cliente", "elle") doivent être introduits une fois explicitement avant d'être repris en pronom.
