...
Deno.serve(async (req) => {
 ...
 try {
   ...
   const action = body.field;
   if (typeof body.token !== "string" || typeof body.post_id !== "string" || typeof body.value !== "string" || !["status", "wording"].includes(action)) return reply({ error: "missing_fields" }, 400);
   const db = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
   const { data, error } = await db.rpc("public_calendar_write", {
     p_token: body.token, p_post_id: body.post_id, p_action: action, p_value: body.value,
