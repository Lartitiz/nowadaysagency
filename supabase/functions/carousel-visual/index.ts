...
MISE EN VALEUR DES MOTS-CLÉS (OPTIONNELLE, selon le sens) :
- Un mot peut recevoir un traitement visuel s'il porte la bascule du propos. Un titre sans effet est parfaitement valide. Ne programme aucune alternance mécanique. Techniques disponibles :
...
- ❌ Décorations ajoutées pour remplir : cartes, pastilles, emojis, pointillés et traits flottants sans rôle éditorial
...
- Layouts selon photo_layout, sans décoration obligatoire :
  · "top_photo" : photo de 650px en haut, texte en dessous sur fond ${ch.color_background}, dimensionné selon la longueur. Le cadrage et l’échelle typographique portent le design.
  · "left_photo" : photo 432px à gauche, texte dans une colonne à droite, avec une vraie gouttière et sans barre décorative.
  · "right_photo" : photo à droite, texte à gauche, sans flèche ni badge ajoutés.
...
- Fond ${ch.color_background}, ou rupture dans la palette de marque si le plan le prévoit. Pas d’alternance blanche automatique.
...
export function buildCoherencePlan(slides: any[], ch: any, darkBrand: boolean): string {
  return describeCarouselDesignPlan(buildCarouselDesignPlan(slides)) + (darkBrand ? `\nLa marque est sombre : les fonds restent dans la gamme de ${ch.color_background}, sans alternance blanche imposée.` : `\nFond principal : ${ch.color_background}. Les changements de fond suivent les ruptures éditoriales, pas la parité du numéro de slide.`);
}
...
async function runParallelChunkGeneration(params: {
  ...
  // The full plan is already in finalUserPrompt, for both single and chunked calls.
  const planBlock = "";
  let doneCount = 0;
...
Adapte le design system ci-dessus au style "${style}". Le style influence l'ambiance mais les règles de design (padding, fonts, badges, barres latérales) restent les mêmes.`;
    ...
    const designPlan = buildCarouselDesignPlan(slides);
    if (!isPhotoCarousel) finalUserPrompt += `\n\n${buildCoherencePlan(slides, ch, darkBrand)}`;
...
Le JSON DOIT inclure deux champs au top-level :
{
  ...
  "slides_invariants": {
    ...
    "layouts_used": ["opening", "essay", ...],  // 1 entrée par slide, selon le plan
    "motif": "editorial"
  }
}
...
6. Les éléments essentiels restent dans les marges sûres, sans étirement artificiel.
