...
serve(async (req) => {
  ...
  try {
    ...
    // Pre-gen coaching brief injection
    const preGenBrief = params.pre_gen_brief;
    const PRE_GEN_BLOCK = preGenBrief ? `\nBRIEF DE COACHING PRÉ-GÉNÉRATION :\nL'utilisatrice a échangé avec le coach avant de générer. Voici ce qui en ressort :\n${preGenBrief}\n\nUTILISE CE BRIEF comme base pour la génération. C'est la source la plus fiable sur ce que l'utilisatrice veut.\n\n` : "";

    let systemPrompt = "";
    let userPrompt = "";

    if (action === "generate-all") {
      systemPrompt = `${WEBSITE_PRINCIPLES}\n\n${context}${PRE_GEN_BLOCK}\n\nGenere les textes complets pour une page d'accueil, section par section :\n\nSECTION 1 : TITRE (HOOK)\n- 1 titre principal (max 12 mots, percutant, benefice client)\n- 1 sous-titre (1-2 phrases)\n\nSECTION 2 : LE PROBLEME\n- 3 phrases max : accroche empathique + mission + promesse\n\nSECTION 3 : LES BENEFICES\n- 3 phrases max : vision + objectif incarne + promesse\n\nSECTION 4 : L'OFFRE\n- Titre engageant + 4-6 points cles\n\nSECTION 5 : QUI TU ES\n- 3-4 phrases basees sur le storytelling\n\nSECTION 6 : FAQ\n- 6 questions/reponses\n\nSECTION 7 : CTA\n- 3 suggestions\n\nReponds UNIQUEMENT en JSON sans backticks :\n{"titre": "...", "sous_titre": "...", "probleme": "...", "benefices": "...", "offre": "...", "presentation": "...", "faq": [{"question": "...", "reponse": "..."}, ...], "cta": ["...", "...", "..."]}`;
      userPrompt = "Genere toute ma page d'accueil.";

    } else if (action === "pas") {
      systemPrompt = `${WEBSITE_PRINCIPLES}\n\n${context}${PRE_GEN_BLOCK}\n\nGenere les textes complets pour une page d'accueil avec le framework PAS (Probleme - Agitation - Solution). Direct et efficace, sans manipulation.\n\nSECTION 1 : TITRE (HOOK)\n- 1 titre principal (max 12 mots) qui nomme le PROBLEME ou le desir de la cliente\n- 1 sous-titre (1-2 phrases)\n\nSECTION 2 : LE PROBLEME (Probleme + Agitation)\n- Nomme le probleme concret, puis fais ressentir le cout de l'inaction (l'agitation reste HONNETE : on rappelle une frustration deja vecue, on ne cree PAS de peur nouvelle, on ne catastrophise pas, on ne shame pas). 3-4 phrases.\n\nSECTION 3 : LES BENEFICES (la Solution / la vie apres)\n- 3 phrases max : ce que la cliente gagne concretement et emotionnellement.\n\nSECTION 4 : L'OFFRE (la Solution incarnee)\n- Titre engageant + 4-6 points cles.\n\nSECTION 5 : QUI TU ES\n- 3-4 phrases basees sur le storytelling.\n\nSECTION 6 : FAQ\n- 6 questions/reponses (lever les objections).\n\nSECTION 7 : CTA\n- 3 suggestions, directes.\n\nReponds UNIQUEMENT en JSON sans backticks (MEME structure que la page standard) :\n{"titre": "...", "sous_titre": "...", "probleme": "...", "benefices": "...", "offre": "...", "presentation": "...", "faq": [{"question": "...", "reponse": "..."}, ...], "cta": ["...", "...", "..."]}`;
      userPrompt = "Genere toute ma page d'accueil avec le framework PAS.";
