...
//
// Lancer : deno test --allow-env --allow-read supabase/functions/linkedin-coaching/index_test.ts
...
Deno.test("phase questions: succès IA -> logUsage appelé (1 ligne ai_usage)", async () => {
  ...
  try {
    const res = await handler(
      authedRequest(`${TEST_SUPABASE_URL}/functions/v1/linkedin-coaching`, {
        phase: "questions",
        module: "profil",
      }),
    );
    ...
    assertEquals(mock.aiUsageInserts[0].action_type, "linkedin_coaching_questions");
  } finally {
    mock.restore();
  }
});
...
Deno.test("phase questions: échec IA -> fallback renvoyé SANS logUsage (aucune ligne ai_usage)", async () => {
  ...
  try {
    ...
    assertEquals(body.questions.length, 4); // fallback = MODULE_QUESTIONS.profil
