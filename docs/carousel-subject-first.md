...
`writing_version` identifies `subject-first-opus-v3`. The reviewer is `contextual-opus-v3`, using the existing `claude-opus-4-8` model for the bounded exact-patch semantic review only. Generation still respects normal versus quality_max mode; other generators and their models are unchanged. This editorial model change was announced before acceptance testing. Review status remains execution metadata, never a quality certificate. The source-first-only candidate still produced slogans. The independent-copy/pairwise-selection experiment was also rejected: it preferred unsupported prose, adding latency without reliable benefit. That extra two-call architecture has been removed. The current candidate keeps the simpler generation + at-most-two exact-patch reviews, with the existing timeout and factual/quote/layout guards. Supplied slides bypass writing/review.
