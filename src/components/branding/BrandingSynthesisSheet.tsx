...
function SectionCard({ children }: { emoji?: string; title?: string; children: React.ReactNode }) {
  return (
...
function SectionLight({ children }: { emoji?: string; title?: string; children: React.ReactNode }) {
  return (
...
function Tags({ items }: { items: string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-2.5">
      {items.map((item, i) => (
        <span key={i} className="text-sm font-medium px-4 py-2 rounded-full bg-rose-pale text-foreground border border-primary/10">{item}</span>
      ))}
    </div>
  );
}
    ...
    <div className="space-y-4 max-w-full overflow-x-hidden">
      ...
      <div ref={sheetRef} className="bg-card border border-border rounded-2xl space-y-0 max-w-full overflow-hidden" style={{ wordBreak: "break-word", overflowWrap: "anywhere" }}>
        ...
        <div className="relative p-8 sm:p-12 md:p-16 rounded-t-2xl overflow-hidden" style={{ background: "linear-gradient(160deg, #FFF4F8 0%, #FFE561 30%, #ffa7c6 60%, #FFF4F8 100%)", opacity: 0.97 }}>
          ...
          <div className="relative z-10 text-center max-w-lg mx-auto">
            ...
            <div className="mt-8 bg-white/70 backdrop-blur-sm rounded-xl p-4 border border-white/50">
              ...
              {missingParts.length > 0 && <p className="text-xs text-muted-foreground mt-2">Manque : {missingParts.join(", ")}.</p>}
            </div>
          </div>
        </div>
          ...
          <div className="space-y-8">
            {(proposition?.version_final || proposition?.version_one_liner) && (
              <div className="text-center py-4 sm:py-6 px-4">
                <p className="text-xl sm:text-2xl md:text-3xl font-display font-bold text-foreground leading-snug max-w-2xl mx-auto">"{proposition.version_final || proposition.version_one_liner}"</p>
              </div>
            )}
            ...
            {(brand?.tone_keywords || brand?.values) && (() => {
              ...
              const valuesList = Array.isArray(rawValues) ? rawValues.filter(Boolean).map(String) : typeof rawValues === "string" ? rawValues.split(/[,;\n]/).map((s: string) => s.trim()).filter(Boolean) : [];
                ...
                <div className="text-center">
                  ...
                  <div className="flex flex-wrap justify-center gap-2.5">
                    {valuesList.slice(0, 8).map((v: string, i: number) => (
                      <span key={i} className="text-sm font-medium px-4 py-2 rounded-full bg-rose-pale text-foreground border border-primary/10">{v}</span>
                    ))}
                  </div>
                </div>
              );
        ...
        {portrait ? (
          <SectionCard emoji="👤" title="Ma cible">
            <div>
              ...
              {portrait.qui_elle_est?.metier && <p className="text-sm text-muted-foreground">{portrait.qui_elle_est.metier}</p>}
              {portrait.qui_elle_est?.situation && <p className="text-sm text-muted-foreground">{portrait.qui_elle_est.situation}</p>}
              {portrait.qui_elle_est?.ca && <p className="text-sm text-muted-foreground">CA : {portrait.qui_elle_est.ca}</p>}
            </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {portrait.objectifs.map((o: string, i: number) => (
                    <li key={i} className="text-sm text-foreground flex items-start gap-2"><span className="shrink-0">✨</span> <span className="break-words">{o}</span></li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {portrait.ses_mots?.length > 0 && (
              <div>
                ...
                <div className="rounded-lg bg-muted/30 border-l-2 border-primary/30 px-4 py-3">
                  <p className="text-sm text-foreground italic flex items-start gap-2 break-words"><span className="shrink-0">💬</span>"{portrait.ses_mots[0]}"</p>
                </div>
              </div>
            )}
          </SectionCard>
          ...
          <SectionCard emoji="👤" title="Ma cible">
            {persona.step_1_frustrations && (
              <div><p className="text-sm font-semibold text-foreground mb-1">Ses frustrations</p><CollapsibleText text={persona.step_1_frustrations} /></div>
            )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {parseStringList(brand.things_to_avoid).map((item, i) => (
                    <span key={i} className="text-xs bg-muted/50 text-muted-foreground rounded-full px-3 py-1.5 border border-border/50">✕ {item}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="rounded-2xl bg-gradient-to-br from-rose-pale/60 to-card border border-primary/10 p-6 sm:p-8 space-y-5">
                ...
                {summaries?.combats_hook ? (
                  ...
                  <div className="space-y-4">
                    {brand.combat_cause && <div><p className="text-xs text-muted-foreground mb-1">La cause</p><CollapsibleText text={brand.combat_cause} maxChars={200} /></div>}
                    {brand.combat_fights && <div><p className="text-xs text-muted-foreground mb-1">Ce contre quoi je me bats</p><CollapsibleText text={brand.combat_fights} maxChars={150} /></div>}
                    {brand.combat_alternative && <div><p className="text-xs text-muted-foreground mb-1">Mon alternative</p><CollapsibleText text={brand.combat_alternative} maxChars={150} /></div>}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-6">
            {storytelling.pitch_short && (
              <blockquote className="text-center py-2">
                <p className="text-lg sm:text-xl font-display italic text-foreground/80 leading-relaxed max-w-2xl mx-auto">"{storytelling.pitch_short}"</p>
              </blockquote>
            )}
            {storytelling.recap_summary && (() => {
              ...
              return (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {sr.before && <div className="rounded-xl bg-muted/20 p-5 border border-border/40 text-center space-y-2"><span className="text-2xl block">🔵</span><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Avant</p><p className="text-sm text-foreground/80 leading-relaxed">{sr.before}</p></div>}
                  {sr.trigger && <div className="rounded-xl bg-rose-pale/60 p-5 border border-primary/10 text-center space-y-2"><span className="text-2xl block">💥</span><p className="text-xs font-semibold uppercase tracking-wider text-primary/70">Le déclic</p><p className="text-sm text-foreground/80 leading-relaxed">{sr.trigger}</p></div>}
                  {sr.after && <div className="rounded-xl bg-muted/20 p-5 border border-border/40 text-center space-y-2"><span className="text-2xl block">🌱</span><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Après</p><p className="text-sm text-foreground/80 leading-relaxed">{sr.after}</p></div>}
                </div>
              );
            })()}
              ...
              <div className="text-center">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {[strategy.pillar_major, strategy.pillar_minor_1, strategy.pillar_minor_2, strategy.pillar_minor_3].filter(Boolean).map((pillar, i) => (
                    <span key={i} className="text-sm font-semibold px-5 py-2.5 rounded-full bg-primary/10 text-primary border border-primary/20">{pillar}</span>
                  ))}
                </div>
              </div>
            )}
                ...
                return (
                  <div className="rounded-xl bg-muted/10 border border-border/40 p-5">
                    ...
                    <div className="space-y-3">
                      {mix.visibility != null && (
                        <div>
                          <div className="flex justify-between text-xs mb-1"><span className="text-foreground/70">Visibilité</span><span className="font-medium">{Math.round((mix.visibility / 10) * 100)}%</span></div>
                          <div className="h-2 bg-border/30 rounded-full overflow-hidden"><div className="h-full bg-[hsl(var(--obj-visibilite))] rounded-full" style={{ width: `${(mix.visibility / 10) * 100}%` }} /></div>
                        </div>
                      )}
                      {mix.trust != null && (
                        <div>
                          <div className="flex justify-between text-xs mb-1"><span className="text-foreground/70">Confiance</span><span className="font-medium">{Math.round((mix.trust / 10) * 100)}%</span></div>
                          <div className="h-2 bg-border/30 rounded-full overflow-hidden"><div className="h-full bg-[hsl(var(--obj-confiance))] rounded-full" style={{ width: `${(mix.trust / 10) * 100}%` }} /></div>
                        </div>
                      )}
                      {mix.sales != null && (
                        <div>
                          <div className="flex justify-between text-xs mb-1"><span className="text-foreground/70">Vente</span><span className="font-medium">{Math.round((mix.sales / 10) * 100)}%</span></div>
                          <div className="h-2 bg-border/30 rounded-full overflow-hidden"><div className="h-full bg-[hsl(var(--obj-vente))] rounded-full" style={{ width: `${(mix.sales / 10) * 100}%` }} /></div>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <SectionLight emoji="📱" title="Mes canaux">
          <div className="flex flex-wrap gap-2">
            {allChannels.map((ch) => {
              ...
              return (
                <span key={ch.id} className={`text-sm px-3 py-1.5 rounded-full ${active ? "bg-rose-pale text-foreground font-medium" : "bg-muted/40 text-muted-foreground/50"}`}>
                  {active ? "✅" : "🔜"} {ch.label}
                </span>
              );
            })}
          ...
          {planConfig && (
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              {planConfig.daily_time && <span>⏰ Temps dispo : {planConfig.daily_time < 60 ? `${planConfig.daily_time} min/jour` : `${Math.round(planConfig.daily_time / 60)}h/semaine`}</span>}
              {planConfig.monthly_goal && <span>🎯 Objectif : {planConfig.monthly_goal}</span>}
            </div>
          )}
        </SectionLight>
        ...
        {brandingAudit ? (
            ...
            {brandingAudit.score_global != null && (
              <div>
                ...
                <Progress
                  ...
                  className={`h-3 ${brandingAudit.score_global >= 75 ? "[&>div]:bg-[hsl(var(--obj-vente))]" : brandingAudit.score_global >= 50 ? "[&>div]:bg-[hsl(var(--obj-confiance))]" : "[&>div]:bg-destructive"}`}
                />
                <p className="text-xs text-muted-foreground mt-1">{new Date(brandingAudit.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</p>
              </div>
            )}
          ...
          <SectionLight emoji="🔍" title="Mon dernier audit">
            <EmptySection message="Tu n'as pas encore fait ton audit." linkLabel="Lancer un audit →" link="/branding/audit" />
          </SectionLight>
        )}
