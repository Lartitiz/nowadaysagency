...
import { isAcceptedFile, ACCEPTED_MIME_TYPES } from "@/lib/file-extractors";
...
export default function BrandingImport({ onAnalyze, onSkip, loading = false }: BrandingImportProps) {
  ...
  const handleFiles = useCallback((newFiles: FileList | File[]) => {
    ...
    for (const file of Array.from(newFiles)) {
      ...
      if (!["pdf", "docx", "doc", "txt", "md", "png", "jpg", "jpeg"].includes(ext || "")) {
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      ...
      >
        ...
        <motion.div
          variants={fadeUp}
          className="bg-card rounded-[20px] border border-border shadow-card p-6 sm:p-8 mb-4"
        >
          <motion.div variants={stagger} className="space-y-4">
            ...
            <motion.div variants={fadeUp}>
              ...
              <Input
                type="url"
                placeholder="URL de ton profil ou ta page"
                value={linkedin}
          ...
          <p className="text-[13px] text-muted-foreground text-center mt-5 italic">
            Même un seul lien suffit pour commencer !
          </p>
        </motion.div>
            ...
            <CollapsibleTrigger className="w-full text-left">
              <div className="rounded-[20px] border border-dashed border-border bg-card/60 hover:bg-card transition-colors p-4 cursor-pointer">
                <p className="text-[13px] text-muted-foreground flex items-center gap-2">
                  ...
                  Tu as aussi des documents ? (plaquette, business plan, présentation...)
                </p>
              </div>
            </CollapsibleTrigger>
            <CollapsibleContent className="mt-3">
              <div
                className={`rounded-[16px] border-2 border-dashed transition-colors p-6 text-center ${
              ...
              >
                ...
                <p className="text-[11px] text-muted-foreground">
                  PDF, Word, texte ou images • Max 5 fichiers, 10 Mo chacun
                </p>
                <p className="text-[11px] text-muted-foreground/70 mt-1 italic">
                  Ça m'aidera à mieux comprendre ton projet
                </p>
                <input
                  ref={fileInputRef}
                  type="file"
        ...
        <motion.div variants={fadeUp} className="text-center space-y-4">
          ...
          <button
            onClick={onSkip}
            className="block mx-auto font-mono-ui text-[13px] text-muted-foreground hover:text-foreground transition-colors"
          >
