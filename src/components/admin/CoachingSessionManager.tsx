...
import {
  FOCUS_TOPICS, getFocusLabel, getFocusIcon, getSessionTypeIcon,
...
import type { ProgramWithProfile, SessionData, DeliverableData, ActionData } from "./admin-coaching-types";
import { SESSION_TYPES, DURATION_OPTIONS, STATUS_OPTIONS, formatDuration } from "./admin-coaching-types";
import { InlineField, VoiceTextarea, DashboardMessageEditor, DeleteClientDialog, PauseDialog } from "./AdminSharedComponents";
...
export default function CoachingSessionManager({ program, sessions: initialSessions, onBack, onReload }: Props) {
  ...
  const showSaved = (field: string) => { setSavedField(field); setTimeout(() => setSavedField(null), 1200); };
  ...
  const addSession = async (data: Partial<SessionData>) => {
    ...
    const { data: inserted } = await (supabase.from("coaching_sessions" as any).insert({
      program_id: program.id, session_number: maxNum + 1,
      phase: data.session_type === "focus" ? "focus" : "strategy",
      title: data.title || "À définir ensemble", session_type: data.session_type || "focus",
      focus_topic: data.focus_topic || null, duration_minutes: data.duration_minutes || 60,
      status: "scheduled", scheduled_date: data.scheduled_date || null,
  ...
  const addDeliverable = async (assignedSessionId?: string) => {
    const { data: d } = await (supabase.from("coaching_deliverables" as any).insert({ program_id: program.id, title: "Nouveau livrable", status: "pending", assigned_session_id: assignedSessionId || null } as any).select().single() as any);
  ...
  const addAction = async (sessionId?: string) => {
    const { data: a } = await (supabase.from("coaching_actions" as any).insert({ program_id: program.id, session_id: sessionId || null, title: "Nouvelle action" } as any).select().single() as any);
    ...
    <div className="min-h-screen bg-background pb-20 lg:pb-8">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 animate-fade-in">
        ...
        <section className="rounded-2xl border border-border bg-card p-5 mb-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="mt-3 pt-3 border-t border-border">
              ...
              <DashboardMessageEditor value={(program as any).dashboard_message || ""} onSave={async (msg: string) => { await updateProgram("dashboard_message", msg); }} />
            </div>
          </div>
        </section>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={sessions.map(s => s.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-2">
                {sessions.map(s => (
                  <SortableSessionRow key={s.id} session={s} onEdit={() => setEditingSession(s)} savedField={savedField} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
        ...
        <section className="rounded-2xl border border-border bg-card p-5 mb-6">
          ...
          <div className="space-y-2">
            {deliverables.map(d => (
              <AdminDeliverableRow key={d.id} deliverable={d} sessions={sessions} onUpdate={updateDeliverable} onDelete={deleteDeliverable} onUpload={uploadFile} />
            ))}
          </div>
        ...
        {/* ── ZONE DANGER ── */}
        <section className="rounded-2xl border border-destructive/30 bg-destructive/5 p-5 mt-6">
...
function SortableSessionRow({ session, onEdit, savedField }: { session: SessionData; onEdit: () => void; savedField: string | null }) {
...
function AdminDeliverableRow({ deliverable, sessions, onUpdate, onDelete, onUpload }: {
  deliverable: DeliverableData; sessions: SessionData[]; onUpdate: (id: string, updates: Record<string, any>) => void; onDelete: (id: string) => void; onUpload: (id: string, file: File) => void;
}) {
  const fileRef = useRef<HTMLInputElement>(null);
  const isDelivered = deliverable.status === "delivered";
  const [editTitle, setEditTitle] = useState(deliverable.title);
