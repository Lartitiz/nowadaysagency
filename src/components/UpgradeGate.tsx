...
const UPGRADE_MESSAGES: Partial<Record<Feature, string>> = {
  generation_unlimited: "Tu as utilisé tes crédits IA ce mois-ci. Ils se renouvellent le 1er du mois, ou tu peux passer au Premium pour une création illimitée.",
  audit_unlimited: "Tu as utilisé tes audits IA ce mois-ci. Ils se renouvellent le 1er du mois, ou tu peux passer au Premium pour une création illimitée.",
...
export default function UpgradeGate({ feature, children, fallback }: UpgradeGateProps) {
  ...
  const message = UPGRADE_MESSAGES[feature] || "Tu as atteint ta limite de crédits IA ce mois. Passe au Premium pour une création illimitée.";
