...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, onChange, ...props }, ref) => {
  ...
  return (
    <textarea
      className={cn(
        "flex min-h-[200px] w-full rounded-[10px] border-2 border-input bg-card px-4 py-3 text-base font-body ring-offset-background placeholder:text-placeholder placeholder:italic focus-visible:outline-none focus-visible:border-primary focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 transition-colors duration-[250ms] resize-vertical",
        className,
      )}
      ref={(node) => {
