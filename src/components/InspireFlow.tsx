...
export default function InspireFlow() {
  const { user } = useAuth();
  ...
  const fetchFromLink = async () => {
    ...
    try {
      ...
      if (data?.caption) {
        ...
        if (data.author) setScreenshotContext(`Post de @${data.author}`);
        setTab("text");
  ...
  const addFiles = (newFiles: FileList | File[]) => {
    ...
    setFiles((prev) => {
      ...
      if (prev.length + valid.length > MAX_FILES) toast.error(`Maximum ${MAX_FILES} fichiers.`);
      return combined;
    });
  };
  ...
  const analyze = useCallback(async () => {
    ...
    if (isScreenshot) {
      if (files.length === 0) { toast.error("Ajoute au moins un screenshot."); return; }
    } else {
      if (!user || !sourceText.trim() || sourceText.trim().length < 20) { toast.error("Colle au moins 20 caractères de contenu."); return; }
    }
    setLoading(true);
    setResult(null);
    try {
      ...
      if (isScreenshot) {
        ...
        body = { source_type: "screenshot", images: imageData, context: screenshotContext.trim() || undefined };
      } else {
  ...
  const viewHistoryItem = (item: HistoryItem) => {
    ...
    if (item.analysis && item.adapted_content) {
      setResult({ analysis: item.analysis, adapted_content: item.adapted_content, format: item.recommended_format || "", objective: item.objective || "", pillar: item.pillar || "" });
  ...
  const canAnalyze = tab === "screenshot" ? files.length > 0 : sourceText.trim().length >= 20;
        ...
        <CardContent className="p-5">
          <Tabs value={tab} onValueChange={setTab}>
            ...
            <TabsContent value="link">
              ...
              <Button variant="outline" size="sm" className="mt-3" onClick={fetchFromLink} disabled={fetchingLink || !sourceUrl.trim()}>
            ...
            <TabsContent value="screenshot">
              ...
              >
                ...
                <p className="text-sm font-medium text-foreground">Glisse tes screenshots ici</p>
                <p className="text-xs text-muted-foreground mt-1">Post, carrousel, reel, story… (max {MAX_FILES} fichiers, 10 Mo chacun)</p>
                <p className="text-xs text-muted-foreground mt-0.5">JPG, PNG, WebP, PDF</p>
                <input
                  ref={fileInputRef}
                  type="file"
          ...
          <Button className="mt-4 w-full" size="lg" onClick={analyze} disabled={loading || !canAnalyze}>
            ...
            {history.map((item) => (
              <div key={item.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-3">
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-muted-foreground">{new Date(item.created_at).toLocaleDateString("fr-FR")}</p>
                  <p className="text-sm text-foreground truncate">{item.source_text?.slice(0, 80) || item.source_url || "..."}</p>
                </div>
                <div className="flex gap-1 ml-3 shrink-0">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => {
                    if (item.adapted_content) { navigator.clipboard.writeText(item.adapted_content); toast.success("Copié !"); }
                  }}>
