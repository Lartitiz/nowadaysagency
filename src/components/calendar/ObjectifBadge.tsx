...
export function ObjectifBadge({ objectif, size = "sm" }: { objectif: string | null; size?: "sm" | "md" }) {
  ...
  if (size === "sm") {
    return (
      <span
        className="inline-flex items-center gap-0.5 rounded-pill px-1.5 py-px text-2xs font-semibold shrink-0"
        style={{
  ...
  return (
    <span
      className="inline-flex items-center gap-1 rounded-pill px-2 py-0.5 text-2xs font-semibold"
      style={{
        backgroundColor: `hsl(var(--${obj.cssVar}-bg))`,
        color: `hsl(var(--${obj.cssVar}))`,
      }}
