...
const state = vi.hoisted(()=>({scope:'a',owner:'owner',ownerError:false,reload:vi.fn(),ready:true,fail:'',profile:{first_name:'Owner'},brand:{mission:'Mission'},pending:null as null|((value:any)=>void),slow:false,calls:[] as any[]}));
vi.mock('@/contexts/AuthContext',()=>({useAuth:()=>({user:{id:'manager'}})}));
vi.mock('@/hooks/use-workspace-query',()=>({useWorkspaceFilter:()=>({column:'workspace_id',value:state.scope}),useProfileOwner:()=>({userId:state.owner,error:state.ownerError,reload:state.reload}),useWorkspaceReady:()=>state.ready}));
...
beforeEach(()=>{state.scope='a';state.owner='owner';state.ownerError=false;state.reload.mockClear();state.ready=true;state.fail='';state.pending=null;state.slow=false;state.calls=[]});
...
it('owner lookup errors offer retry without a permanent spinner',async()=>{state.owner='';state.ownerError=true;const {result,rerender}=renderHook(()=>useSynthesisFetch());await waitFor(()=>expect(result.current.loadError).toContain('propriétaire'));expect(result.current.loading).toBe(false);await act(()=>result.current.loadData());expect(state.reload).toHaveBeenCalledOnce();state.owner='owner';state.ownerError=false;rerender();await waitFor(()=>expect(result.current.data).not.toBeNull())});
